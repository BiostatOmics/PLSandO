...
#'   \item \code{cv_results}: Detailed results from the cross-validation procedure. Including the results obtained for each repetition for the R2, Q2, PRESS, RMSE, F1score, MCC, Etot (Total error), coef_cv (Coefficients obtained in the cross-validation), BER, Specificity and Sensitivity.
...
#' @param selVars Numeric (0 to 1). Percentage of variables to display in loading, correlation, weights or biplots, selected by their importance ("contrib" or "cos2"). Also aplicable to R2X and R2Y. Useful for decluttering plots with many variables.
...
plsdaPlot = function(x,
  ...
  if(!(type %in% c("ncomp","R2vsQ2", "scoresX",  "loadingsX", "scoresY", "loadingsY", "weights", "linearity", "overfitting", "R2X", "R2Y", "corrX", "corrY", "biplot", "coef"))) return(stop('Please use one of: R2vsQ2, scoresX, loadingsX, scoresY, loadingsY, weights, linearity, overffiting, R2X, R2Y, corrX, corrY, biplot.'))
...
plsdaVarSel = function(x,
  ...
  return(plsVarSel(x,type,rep,threshold,selVars, col))
}
